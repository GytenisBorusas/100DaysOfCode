...
def game():
    ...
    print(f"Compare A: {random_entry_A['name']}, {random_entry_A['description']}, from {random_entry_A['country']}, ")
    print(f"****** {random_entry_A['follower_count']}")
    print(vs)
    random_entry_B = random.choice(data)
    print(f"Against B: {random_entry_B['name']}, {random_entry_B['description']}, from {random_entry_B['country']}, ")
    print(f"****** {random_entry_B['follower_count']}")
        
    while end_of_game_flag == False:
        users_input = input("Who has more followers? Type 'A' or 'B': ") 
        
        if users_input == 'A' and (random_entry_A['follower_count'] > random_entry_B['follower_count']):
            user_score += 1
        ...
        elif users_input == 'A' and (random_entry_A['follower_count'] < random_entry_B['follower_count']):
            end_of_game_flag = True
        elif users_input == 'B' and (random_entry_A['follower_count'] > random_entry_B['follower_count']):
            end_of_game_flag = True
        elif users_input == 'B' and (random_entry_A['follower_count'] < random_entry_B['follower_count']):
            user_score += 1
            random_entry_A = random_entry_B
